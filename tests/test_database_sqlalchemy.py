...
class TestVendorModel:
    ...
    def test_create_vendor(self, session):
        ...
        vendor = Vendor(
            name="Card Kingdom", url="https://www.cardkingdom.com", is_active=True
        )
        session.add(vendor)
        session.flush()
...
class TestDatabaseMigration:
    ...
    def old_db_path(self):
        ...
        # Insert test data
        cursor.execute(
            "INSERT INTO cards (name, set_name, foil, condition, quantity) VALUES (?, ?, ?, ?, ?)",
            ("Test Card", "Test Set", False, "NM", 4),
        )

        cursor.execute(
            "INSERT INTO vendors (name, url, is_active) VALUES (?, ?, ?)",
            ("Test Vendor", "https://test.com", True),
        )

        cursor.execute(
            "INSERT INTO price_cache (card_name, vendor_name, price, condition, foil, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
            (
                "Test Card",
                "Test Vendor",
                10.50,
        ...
        cursor.execute(
            "INSERT INTO dashboard_cache (data, expires_at) VALUES (?, ?)",
            (
                '{"test": "data"}',
