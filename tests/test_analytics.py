    ...
    def setup_method(self):
        ...
        self.sample_prices = [
            CardPrices(
                ...
                prices={
                    "Star City Games": PriceData(vendor="Star City Games", price=10.0, condition="NM"),
                    "Card Kingdom": PriceData(vendor="Card Kingdom", price=12.0, condition="LP"),
                },
                best_price=12.0,
            ...
            CardPrices(
                ...
                prices={
                    "Star City Games": PriceData(vendor="Star City Games", price=25.0, condition="NM"),
                    "Card Kingdom": PriceData(vendor="Card Kingdom", price=30.0, condition="LP"),
                },
                best_price=30.0,
                best_vendor="Card Kingdom"
            ),
        ...
        mock_history_data = [
            {
                ...
                "prices": {"Star City Games": {"price": 1.0 + i * 0.1, "condition": "NM"}},
    ...
    def setup_method(self):
        ...
        self.sample_prices = [
            ...
            CardPrices(
                ...
                prices={
                    "Star City Games": PriceData(vendor="Star City Games", price=5.0, condition="NM"),
                    "Card Kingdom": PriceData(vendor="Card Kingdom", price=6.0, condition="LP"),
                },
                best_price=6.0,
                best_vendor="Card Kingdom"
            ),
    ...
    def test_generate_analytics_report(self):
        ...
        analysis = {
            ...
            "risk_assessment": {
                ...
                "risk_categories": {
                    "low_risk": {"percentage": 60.0, "value": 28.8, "count": 2, "weighted_risk": 12.6},
                    "medium_risk": {"percentage": 25.0, "value": 12.0, "count": 1, "weighted_risk": 15.0},
                    "high_risk": {"percentage": 15.0, "value": 7.2, "count": 0, "weighted_risk": 0.0}
                },
...
class TestCLIIntegration:
    def test_cli_analytics_command(self):
        result = subprocess.run([sys.executable, 'main.py', 'analytics', '--use-mock'], capture_output=True, text=True)
        assert result.returncode == 0
        assert 'COLLECTION ANALYTICS REPORT' in result.stdout

    def test_cli_hot_cards_command(self):
        result = subprocess.run([sys.executable, 'main.py', 'hot-cards', '--use-mock'], capture_output=True, text=True)
        assert result.returncode == 0
        assert 'HOT CARDS ANALYSIS REPORT' in result.stdout or 'No hot cards detected' in result.stdout

    def test_cli_recommendations_command(self):
        result = subprocess.run([sys.executable, 'main.py', 'recommendations', '--use-mock'], capture_output=True, text=True)
        assert result.returncode == 0
        assert 'STRATEGIC RECOMMENDATIONS REPORT' in result.stdout

    def test_cli_vendor_health_command(self):
        result = subprocess.run([sys.executable, 'main.py', 'vendor-health', '--use-mock'], capture_output=True, text=True)
