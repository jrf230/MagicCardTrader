        ...
        try:
            ...
            if buylist_data:
                ...
                if buylist_data.all_conditions.get(
                    "buylist_price"
                ) and buylist_data.all_conditions.get("sell_price"):
                    print(f"  Buylist: ${buylist_data.all_conditions['buylist_price']}")
                    print(f"  Sell: ${buylist_data.all_conditions['sell_price']}")
                    print(
                        f"  Vendors Aggregated: {len(buylist_data.all_conditions['vendors_aggregated'])}"
                    )
                    print(
                        f"  Total Vendors Available: {buylist_data.all_conditions['total_vendors']}"
                    )
            ...
            if sell_data:
                ...
                print(
                    f"  Vendors Aggregated: {len(sell_data.all_conditions['vendors_aggregated'])}"
                )
            else:
...
def test_beatthebuylist_with_sell_price():
    ...
    try:
        ...
        if combined_data["buylist"]:
            ...
            print(
                f"  Vendors Aggregated: {len(buylist.all_conditions['vendors_aggregated'])}"
            )
        else:
        ...
        if combined_data["sell"]:
            ...
            print(
                f"  Vendors Aggregated: {len(sell.all_conditions['vendors_aggregated'])}"
            )
        else:
            print("✗ No sell data")
