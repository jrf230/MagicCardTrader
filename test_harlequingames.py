#!/usr/bin/env python3
"""Test script for Harlequin Games scraper."""
...
def test_harlequingames_scraper():
    """Test the Harlequin Games scraper with various cards."""

    scraper = HarlequinGamesScraper()
    ...
    print("Testing Harlequin Games Scraper")
    print("=" * 50)
    ...
    for card in test_cards:
        ...
        try:
            ...
            if buylist_data:
                ...
                print(
                    f"  Data Source: {buylist_data.all_conditions.get('data_source', 'web_scraped')}"
                )

                # Check if we have both cash and credit prices
                if buylist_data.all_conditions.get(
                    "cash_price_usd"
                ) and buylist_data.all_conditions.get("credit_price_usd"):
                    print(
                        f"  Cash (USD): ${buylist_data.all_conditions['cash_price_usd']}"
                    )
                    print(
                        f"  Credit (USD): ${buylist_data.all_conditions['credit_price_usd']}"
                    )
                    print(
                        f"  Cash (GBP): £{buylist_data.all_conditions['cash_price_gbp']}"
                    )
                    print(
                        f"  Credit (GBP): £{buylist_data.all_conditions['credit_price_gbp']}"
                    )
                    print(
                        f"  Exchange Rate: 1 GBP = ${buylist_data.all_conditions['exchange_rate']} USD"
                    )
            else:
            ...
            if sell_data:
                ...
                print(f"  Sell (GBP): £{sell_data.all_conditions['sell_price_gbp']}")
                print(f"  Sell (USD): ${sell_data.all_conditions['sell_price_usd']}")
            else:
...
def test_harlequingames_with_sell_price():
    ...
    print("\nTesting Harlequin Games with combined buylist/sell prices")
    print("=" * 60)
    ...
    try:
        ...
        if combined_data["buylist"]:
            ...
            print(
                f"  Cash: ${buylist.all_conditions['cash_price_usd']} (£{buylist.all_conditions['cash_price_gbp']})"
            )
            print(
                f"  Credit: ${buylist.all_conditions['credit_price_usd']} (£{buylist.all_conditions['credit_price_gbp']})"
            )
        else:
        ...
        if combined_data["sell"]:
            ...
            print(
                f"  Sell: ${sell.all_conditions['sell_price_usd']} (£{sell.all_conditions['sell_price_gbp']})"
            )
        else:
            print("✗ No sell data")
...
if __name__ == "__main__":
    test_harlequingames_scraper()
    test_harlequingames_with_sell_price()
