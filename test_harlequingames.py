    ...
    for card in test_cards:
        ...
        try:
            ...
            if buylist_data:
                ...
                if buylist_data.all_conditions.get('cash_price_usd') and buylist_data.all_conditions.get('credit_price_usd'):
                    print(f"  Cash (USD): ${buylist_data.all_conditions['cash_price_usd']}")
                    print(f"  Credit (USD): ${buylist_data.all_conditions['credit_price_usd']}")
                    print(f"  Cash (GBP): £{buylist_data.all_conditions['cash_price_gbp']}")
                    print(f"  Credit (GBP): £{buylist_data.all_conditions['credit_price_gbp']}")
                    print(f"  Exchange Rate: 1 GBP = ${buylist_data.all_conditions['exchange_rate']} USD")
            else:
...
def test_harlequingames_with_sell_price():
    ...
    try:
        ...
        if combined_data["buylist"]:
            ...
            print(f"  Cash: ${buylist.all_conditions['cash_price_usd']} (£{buylist.all_conditions['cash_price_gbp']})")
            print(f"  Credit: ${buylist.all_conditions['credit_price_usd']} (£{buylist.all_conditions['credit_price_gbp']})")
        else:
        ...
        if combined_data["sell"]:
            ...
            print(f"  Sell: ${sell.all_conditions['sell_price_usd']} (£{sell.all_conditions['sell_price_gbp']})")
        else:
            print("✗ No sell data")
