...
class HotCardDetector:
    ...
    def detect_hot_cards(
        self, card_prices_list: List[CardPrices], days: int = 7
    ) -> List[Dict]:
        ...
        """
        logger.info(
            f"Detecting hot cards for {len(card_prices_list)} cards over {days} days"
        )
        ...
        for card_prices in card_prices_list:
            ...
            if (
                analysis
                and analysis["hot_score"] >= self.config["confidence_threshold"]
            ):
    ...
    def _analyze_price_trends(self, history: List[Dict]) -> Optional[Dict]:
        ...
        for i in range(1, len(best_prices)):
            ...
            change_percent = (
                (change / best_prices[i - 1]) * 100 if best_prices[i - 1] > 0 else 0
            )
        ...
        sustained_movement = self._check_sustained_movement(
            best_prices, trend_direction
        )
    ...
    def _calculate_risk_adjustment(self, card: Card, price_analysis: Dict) -> float:
        ...
        if card.set_name and any(
            keyword in card.set_name.lower() for keyword in ["core set", "standard"]
        ):
    ...
    def generate_hot_cards_report(self, hot_cards: List[Dict]) -> str:
        ...
        total_value_change = sum(
            card["price_change_amount"] * card["card"].quantity for card in hot_cards
        )
        avg_change_percent = statistics.mean(
            card["price_change_percent"] for card in hot_cards
        )
        ...
        report_lines.append(
            f"Average Hot Score: {statistics.mean(card['hot_score'] for card in hot_cards):.2f}"
        )
        report_lines.append("")
        ...
        for i, card_analysis in enumerate(hot_cards[:10], 1):  # Top 10
            ...
            report_lines.append(
                f"   Price Change: {card_analysis['price_change_percent']:+.1f}% (${card_analysis['price_change_amount']:+.2f})"
            )
            report_lines.append(
                f"   Trend: {card_analysis['trend_direction'].replace('_', ' ').title()}"
            )
            report_lines.append(f"   Recommendation: {card_analysis['recommendation']}")
            report_lines.append(
                f"   Risk Factors: {', '.join(card_analysis['risk_factors']) if card_analysis['risk_factors'] else 'None'}"
            )
            report_lines.append(f"   Quantity: {card.quantity}")
            report_lines.append(
                f"   Total Value Impact: ${card_analysis['price_change_amount'] * card.quantity:+.2f}"
            )
