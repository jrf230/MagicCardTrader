...
class RecommendationEngine:
    ...
    def generate_recommendations(
        self, card_prices_list: List[CardPrices], collection_summary: CollectionSummary
    ) -> Dict[str, List[Dict]]:
    ...
    def _generate_sell_recommendations(
        self,
        card_prices_list: List[CardPrices],
        ...
        for card_prices in card_prices_list:
            ...
            sell_analysis = self._analyze_sell_potential(
                card, card_prices, hot_card_names
            )

            if (
                sell_analysis
                and sell_analysis["confidence"] >= self.config["confidence_threshold"]
            ):
    ...
    def _analyze_sell_potential(
        self, card: Card, card_prices: CardPrices, hot_card_names: set
    ) -> Optional[Dict]:
        ...
        historical_prices = [
            entry["best_price"] for entry in history if entry.get("best_price")
        ]
        ...
        older_prices = (
            historical_prices[:-3] if len(historical_prices) > 3 else historical_prices
        )
        ...
        if volatility > 20:
            ...
            confidence += 0.1
            reasoning.append(
                f"High volatility ({volatility:.1f}%) - good selling opportunity"
            )
    ...
    def _generate_buy_recommendations(
        ...
        for card_prices in card_prices_list:
            ...
            buy_analysis = self._analyze_buy_potential(
                card, card_prices, hot_card_names
            )

            if (
                buy_analysis
                and buy_analysis["confidence"] >= self.config["confidence_threshold"]
            ):
    ...
    def _analyze_buy_potential(
        ...
        potential_savings = (
            (avg_price - current_price) if current_price < avg_price else 0
        )
    ...
    def _generate_hold_recommendations(
        ...
        for card_prices in card_prices_list:
            ...
            hold_analysis = self._analyze_hold_potential(
                card, card_prices, hot_card_names
            )

            if (
                hold_analysis
                and hold_analysis["confidence"] >= self.config["confidence_threshold"]
            ):
    ...
    def _analyze_hold_potential(
        ...
        """
        # Get price history
        history = self.price_history.get_card_history(
            card.name, card.set_name, days=self.config["hold_threshold_days"]
        )
        ...
        expected_value = current_price * card.quantity
        potential_growth = (
            (long_term_trend / 100) * expected_value if long_term_trend > 0 else 0
        )
    ...
    def _assess_sell_risk(
        self, card: Card, price_change_percent: float, volatility: float
    ) -> str:
    ...
    def _generate_recommendation_summary(
        self, sell_recs: List[Dict], buy_recs: List[Dict], hold_recs: List[Dict]
    ) -> Dict:
        ...
        return {
            ...
            "net_potential_value": total_potential_profit
            + total_potential_savings
            + total_potential_growth,
        }
    ...
    def generate_recommendation_report(self, recommendations: Dict) -> str:
        ...
        report_lines.append(
            f"Sell: {summary['sell_count']} cards (${summary['total_sell_value']:.2f})"
        )
        report_lines.append(
            f"Buy: {summary['buy_count']} cards (${summary['total_buy_value']:.2f})"
        )
        report_lines.append(
            f"Hold: {summary['hold_count']} cards (${summary['total_hold_value']:.2f})"
        )
        report_lines.append("")
        report_lines.append(
            f"Potential Profit from Selling: ${summary['total_potential_profit']:.2f}"
        )
        report_lines.append(
            f"Potential Savings from Buying: ${summary['total_potential_savings']:.2f}"
        )
        report_lines.append(
            f"Potential Growth from Holding: ${summary['total_potential_growth']:.2f}"
        )
        report_lines.append(
            f"Net Potential Value: ${summary['net_potential_value']:.2f}"
        )
        report_lines.append("")
        ...
        if recommendations["sell"]:
            ...
            for i, rec in enumerate(recommendations["sell"][:10], 1):
                ...
                report_lines.append(
                    f"   Potential Profit: ${rec['potential_profit']:.2f}"
                )
        ...
        if recommendations["hold"]:
            ...
            for i, rec in enumerate(recommendations["hold"][:10], 1):
                ...
                report_lines.append(f"   Current Price: ${rec['current_price']:.2f}")
                report_lines.append(
                    f"   Potential Growth: ${rec['potential_growth']:.2f}"
                )
                report_lines.append(f"   Risk Level: {rec['risk_level'].title()}")
                report_lines.append(f"   Reasoning: {'; '.join(rec['reasoning'])}")
