...
def get_api_stats():
    ...
    return {
        ...
        'success_rate': ((api_stats['total_requests'] - api_stats['errors']) / 
...
def api_recommendations():
    ...
    try:
        force_refresh = request.args.get('force_refresh', 'false').lower() == 'true'
        recommendations_data = api_service.get_recommendations(force_refresh=force_refresh)
...
def api_search_cards():
    """Search for cards by name, set, rarity"""
    try:
        query = request.args.get('q', '').lower()
        set_name = request.args.get('set', '').lower()
        ...
        # In a real implementation, you'd search against a card database
        mock_results = [
            {
                'name': 'Rhystic Study',
                'set_name': 'Prophecy',
                'set_code': 'PCY',
                'rarity': 'Rare',
                'image_url': 'https://example.com/rhystic-study.jpg'
            },
            {
                'name': 'Sol Ring',
        ...
        filtered_results = []
        for card in mock_results:
            matches = True
            if query and query not in card['name'].lower():
                matches = False
            if set_name and set_name not in card['set_name'].lower():
                matches = False
            if rarity and rarity not in card['rarity'].lower():
                matches = False
            
            if matches:
                filtered_results.append(card)
        
        return jsonify({'cards': filtered_results})
...
def api_card_details():
    ...
    card.foil = foil
    # Get price data from all vendors (existing logic)
    ...
    for vendor, price_data_list in card_price_data.prices.items():
        ...
        for price_data in price_data_list:
            ...
            if card.is_foil() != (getattr(price_data, 'foil', False) or getattr(price_data, 'foil_treatment', None) == 'Foil'):
                continue
            # Separate bid/offer/ask/mid if available
            bid = price_data.all_conditions.get('bid_price') or price_data.all_conditions.get('Bid')
            ask = price_data.all_conditions.get('offer_price') or price_data.all_conditions.get('Offer')
            mid = price_data.all_conditions.get('market_price') or price_data.all_conditions.get('Market')
            # Fallback to price if not available
            if bid is None: bid = price_data.price
            if ask is None: ask = price_data.price
            if mid is None: mid = price_data.price
            vendors_data.append({
                'vendor': vendor,
                'bid': bid,
                'ask': ask,
    ...
    response = {
        ...
        'current_prices': {
            'best_bid': card_price_data.best_bid.price if card_price_data.best_bid else None,
            'best_vendor': card_price_data.best_bid.vendor if card_price_data.best_bid else None,
...
def api_search_cards_autocomplete():
    ...
    try:
        ...
        scryfall_url = f"https://api.scryfall.com/cards/autocomplete?q={quote_plus(query)}"
        ...
        filtered_data = [
            card_name for card_name in data 
...
scheduler.add_job(refresh_prices_and_analytics, 'interval', hours=6, id='refresh_job', replace_existing=True)
scheduler.start()
