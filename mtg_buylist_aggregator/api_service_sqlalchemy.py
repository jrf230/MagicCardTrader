...
from mtg_buylist_aggregator.database_sqlalchemy import (
    get_database_manager, Card, Vendor, Price, Cache, Analytics
)
...
class EnhancedAPIService:
    ...
    def _set_cache(self, key: str, data: Dict[str, Any], duration: Optional[timedelta] = None) -> bool:
        """Set data in cache."""
        try:
            if duration is None:
    ...
    def get_collection(self, force_refresh: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            with self.db_manager.get_session() as session:
                cards = session.query(Card).options(
                    joinedload(Card.prices).joinedload(Price.vendor)
                ).all()
                
                collection_data = []
                for card in cards:
                    # Get latest prices for this card
                    latest_prices = session.query(Price).filter(
                        Price.card_id == card.id
                    ).order_by(desc(Price.timestamp)).limit(10).all()
                    ...
                    if best_buy_price:
                        best_price_entry = next(
                            (p for p in latest_prices if p.price == best_buy_price and p.is_buy_price),
                            None
                        )
                        if best_price_entry:
                    ...
                    card_data = {
                        ...
                        'last_updated': card.updated_at.isoformat() if card.updated_at else None
                    }
    ...
    def get_dashboard_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            cards_with_prices = len([card for card in collection if card['best_buy_price'] is not None])
            
            # Get recent price updates
            with self.db_manager.get_session() as session:
                recent_prices = session.query(Price).order_by(
                    desc(Price.timestamp)
                ).limit(20).options(
                    joinedload(Price.card),
                    joinedload(Price.vendor)
                ).all()
            ...
            dashboard_data = {
                ...
                'summary': {
                    ...
                    'price_coverage': (cards_with_prices / total_cards * 100) if total_cards > 0 else 0
                },
            ...
            return {
                'collection': [],
                'summary': {'total_cards': 0, 'total_value': 0, 'cards_with_prices': 0, 'price_coverage': 0},
                'recent_updates': [],
                'vendor_stats': {},
                'last_updated': datetime.utcnow().isoformat()
            }
    ...
    def _get_vendor_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                ).outerjoin(Price).group_by(Vendor.id).all()
                
                stats = {}
                for vendor in vendor_stats:
                    stats[vendor.name] = {
                        'success_rate': vendor.success_rate,
                        'last_scrape': vendor.last_scrape.isoformat() if vendor.last_scrape else None,
                        'price_count': vendor.price_count,
                        'is_active': vendor.success_rate > 0.5 if vendor.success_rate else False
                    }
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db_manager.get_session() as session:
                ...
                expired_cache_count = session.query(Cache).filter(
                    Cache.expires_at <= datetime.utcnow()
                ).count()
