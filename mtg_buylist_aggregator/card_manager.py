...
from .models import Card, Rarity, FoilTreatment, PromoType, ArtworkVariant, BorderTreatment, CardSize, Language, Condition, Edition
...
class CardManager:
    ...
    def _ensure_csv_exists(self) -> None:
        ...
        if not self.csv_path.exists():
            with open(self.csv_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                writer.writerow([
                    'Card Name', 'Set', 'Set Code', 'Scryfall ID', 'Card Number', 'Rarity', 'Quantity', 
                    'Foil Treatment', 'Condition', 'Promo Type', 'Artwork Variant',
                    'Border Treatment', 'Card Size', 'Language', 'Edition',
                    'Signed', 'Original Printing',
                    'Stamp', 'Serialized Number', 'Is Token', 'Is Emblem', 'Is Other'
                ])
    ...
    def load_collection(self) -> List[Card]:
        ...
        try:
            with open(self.csv_path, 'r', newline='', encoding='utf-8') as csvfile:
                ...
                is_legacy_format = 'Foil' in reader.fieldnames and 'Foil Treatment' not in reader.fieldnames
                    ...
                    try:
                        if is_legacy_format:
                            ...
                            card = Card(
                                ...
                                rarity=self._parse_enum_field(row.get('Rarity', ''), Rarity),
                                quantity=int(row['Quantity']),
                                foil_treatment=self._parse_enum_field(row.get('Foil Treatment', 'Non-foil'), FoilTreatment) or FoilTreatment.NONFOIL,
                                condition=self._parse_enum_field(row.get('Condition', 'Near Mint'), Condition) or Condition.NM,
                                promo_type=self._parse_enum_field(row.get('Promo Type', 'Regular'), PromoType) or PromoType.REGULAR,
                                artwork_variant=self._parse_enum_field(row.get('Artwork Variant', 'Normal'), ArtworkVariant) or ArtworkVariant.NORMAL,
                                border_treatment=self._parse_enum_field(row.get('Border Treatment', 'Normal'), BorderTreatment) or BorderTreatment.NORMAL,
                                card_size=self._parse_enum_field(row.get('Card Size', 'Normal'), CardSize) or CardSize.NORMAL,
                                language=self._parse_enum_field(row.get('Language', 'English'), Language) or Language.ENGLISH,
                                edition=self._parse_enum_field(row.get('Edition', ''), Edition),
                                signed=self._parse_bool_field(row.get('Signed', 'No')),
                                original_printing=self._parse_bool_field(row.get('Original Printing', 'Yes')),
                                stamp=row.get('Stamp', '').strip() or None,
                                serialized_number=row.get('Serialized Number', '').strip() or None,
                                is_token=self._parse_bool_field(row.get('Is Token', 'No')),
                                is_emblem=self._parse_bool_field(row.get('Is Emblem', 'No')),
                                is_other=self._parse_bool_field(row.get('Is Other', 'No'))
                            )
                        cards.append(card)
    ...
    def add_card(self, card: Card) -> bool:
        ...
        for existing_card in cards:
            if existing_card.get_unique_key() == card_key:
                ...
                logger.info(f"Updated quantity for {card.name} ({card.set_name}) to {existing_card.quantity}")
                return self.save_collection(cards)
        
        # Add new card
        cards.append(card)
        logger.info(f"Added {card.quantity}x {card.name} ({card.set_name}) to collection")
        return self.save_collection(cards)
    
    def remove_card(self, name: str, set_name: str, foil: bool = False, quantity: Optional[int] = None, **kwargs) -> bool:
        ...
        for i, card in enumerate(cards):
            if all(getattr(card, key) == value for key, value in search_criteria.items()):
                
                if quantity is None or quantity >= card.quantity:
                    # Remove entire card
                    removed_card = cards.pop(i)
                    logger.info(f"Removed {removed_card.quantity}x {removed_card.name} ({removed_card.set_name}) from collection")
                else:
                    # Reduce quantity
                    cards[i].quantity -= quantity
                    logger.info(f"Reduced quantity for {card.name} ({card.set_name}) by {quantity}")
    ...
    def get_collection_stats(self) -> dict:
        ...
        for card in cards:
            if card.rarity:
                rarity_counts[card.rarity.value] = rarity_counts.get(card.rarity.value, 0) + 1
        ...
        condition_counts = {}
        for card in cards:
            condition_counts[card.condition.value] = condition_counts.get(card.condition.value, 0) + 1
