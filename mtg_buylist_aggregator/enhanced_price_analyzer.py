...
class EnhancedPriceAnalyzer:
    ...
    def analyze_card_market(
        self, card: Card, card_prices: CardPrices
        ...
        try:
            ...
            if "Scryfall" in card_prices.prices:
                scryfall_links = card_prices.prices["Scryfall"].all_conditions.get(
                    "price_links"
                )

            # Determine trend based on available data
            trend_direction, trend_percentage = self._determine_trend(
                ebay_data, tcg_insights
            )

            # Calculate market metrics
            current_market_price = self._calculate_market_price(
                market_prices, tcg_insights
            )
            ...
            risk_score = self._calculate_risk_score(
                market_volatility, price_spread, trend_percentage
            )
            ...
            confidence = self._calculate_confidence(
                card_prices, ebay_data, tcg_insights
            )
    ...
    def _calculate_market_price(
        self, market_prices: List[float], tcg_insights: Optional[Dict]
    ) -> float:
    ...
    def _determine_trend(
        self, ebay_data: Optional[Dict], tcg_insights: Optional[Dict]
    ) -> Tuple[str, float]:
    ...
    def _calculate_risk_score(
        self, volatility: float, spread: float, trend_percentage: float
    ) -> float:
    ...
    def _generate_recommendation(
        self, spread: float, trend: float, risk: float, volume: int
    ) -> str:
    ...
    def _calculate_confidence(
        ...
        ebay_data: Optional[Dict],
        tcg_insights: Optional[Dict],
    ) -> float:
                ...
                serializable_insights.append(
                    {
                        "card": {
                            ...
                            "rarity": (
                                insight.card.rarity.value
                                if insight.card.rarity
                                else None
                            ),
                        },
