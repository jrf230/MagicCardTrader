...
class PriceAnalyzer:
    ...
    def analyze_card_prices(self, card_prices: CardPrices) -> Dict[str, any]:
        ...
        if prices:
            ...
            analysis['best_vendor'] = card_prices.best_bid.vendor if card_prices.best_bid else None
        
        return analysis
    
    def analyze_collection_prices(self, card_prices_list: List[CardPrices]) -> CollectionSummary:
        ...
        logger.info(f"Analysis complete: {summary.cards_with_prices} cards with prices, "
                   f"{summary.cards_without_prices} without prices")
        
        return summary
    
    def find_best_vendor_per_card(self, card_prices_list: List[CardPrices]) -> Dict[str, str]:
    ...
    def calculate_optimal_vendor_mix(self, card_prices_list: List[CardPrices]) -> Dict[str, List[str]]:
    ...
    def generate_price_report(self, card_prices_list: List[CardPrices], 
                            summary: CollectionSummary) -> str:
        ...
        report_lines.append("-" * 40)
        for vendor, total_value in sorted(summary.total_value_by_vendor.items(), 
        ...
        for card_prices in card_prices_list:
            if card_prices.prices:
                ...
                sorted_prices = sorted(card_prices.prices.items(), 
                                     key=lambda x: max(pd.price for pd in x[1]) if x[1] else 0, reverse=True)
                ...
                for vendor, price_data_list in sorted_prices:
                    if price_data_list:
                        ...
                        report_lines.append(f"  {vendor:<15} ${price_data.price:>6.2f} "
    ...
    def export_to_csv(self, card_prices_list: List[CardPrices], 
                     filename: str = "price_report.csv") -> bool:
        ...
        try:
            ...
            with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                # Write header
                writer.writerow([
                    'Card Name', 'Set', 'Quantity', 'Vendor', 'Price', 
                    'Condition', 'Total Value', 'Quantity Limit'
                ])
