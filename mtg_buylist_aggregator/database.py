...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_price_cache_card ON price_cache(card_id)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_price_cache_vendor ON price_cache(vendor)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_price_history_card ON price_history(card_id)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_price_history_date ON price_history(recorded_at)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_dashboard_cache_expires ON dashboard_cache(expires_at)")
    ...
    def add_card(self, card: Card) -> bool:
        ...
        with self._lock:
            with self.get_connection() as conn:
                ...
                """, (card.name, card.set_name, card.foil_treatment.value, card.condition.value))
                ...
                if existing:
                    ...
                    logger.info(f"Updated quantity for {card.name} ({card.set_name}) to {new_quantity}")
                else:
                    # Insert new card
                    cursor.execute("""
                        INSERT INTO cards (
                    ...
                    """, (
                        card.name, card.set_name, card.set_code, card.scryfall_id, card.card_number,
                        card.rarity.value if card.rarity else None, card.quantity, card.foil_treatment.value,
                        card.condition.value, card.promo_type.value, card.artwork_variant.value,
                        card.border_treatment.value, card.card_size.value, card.language.value,
                        card.edition.value if card.edition else None, card.signed, card.original_printing,
                        card.stamp, card.serialized_number, card.is_token, card.is_emblem, card.is_other
                    ))
                    logger.info(f"Added {card.quantity}x {card.name} ({card.set_name}) to database")
    ...
    def remove_card(self, name: str, set_name: str, foil: bool = False, quantity: Optional[int] = None) -> bool:
    ...
    def get_price_cache(self, cards: List[Card], force_refresh: bool = False) -> List[CardPrices]:
                ...
                if price_rows:
                    ...
                    for row in price_rows:
                        ...
                        price_data = PriceData(
                            ...
                            last_price_update=datetime.fromisoformat(row['last_price_update']) if row['last_price_update'] else None,
                            notes=row['notes']
                        )
            ...
            with self.get_connection() as conn:
                ...
                for card_prices in card_prices_list:
                    ...
                    cursor.execute("""
                        ...
                        WHERE name = ? AND set_name = ? AND foil_treatment = ?
                    """, (card_prices.card.name, card_prices.card.set_name, card_prices.card.foil_treatment.value))
                    ...
                    cursor.execute("DELETE FROM price_cache WHERE card_id = ?", (card_id,))
                    ...
                    for vendor, price_data_list in card_prices.prices.items():
                        for price_data in price_data_list:
                            ...
                            """, (
                                card_id, vendor, price_data.price, price_data.price_type,
                                price_data.condition, price_data.quantity_limit,
                                price_data.last_price_update.isoformat() if price_data.last_price_update else None,
                                price_data.notes
                            ))
    ...
    def get_cache_status(self) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("SELECT COUNT(DISTINCT card_id) as cached_cards FROM price_cache")
            ...
            return {
                ...
                'cache_coverage': (cached_cards / total_cards * 100) if total_cards > 0 else 0
            }
    ...
    def set_dashboard_cache(self, data: Dict, cache_key: str = "main", ttl_hours: int = 24) -> None:
    ...
    def clear_dashboard_cache(self, cache_key: str = "main") -> None:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM dashboard_cache WHERE cache_key = ?", (cache_key,))
            conn.commit()
    ...
    def set_analytics_cache(self, analysis_type: str, data: Dict, ttl_hours: int = 6) -> None:
    ...
    def get_collection_stats(self) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("SELECT COUNT(*) as total_cards, SUM(quantity) as total_quantity FROM cards")
            stats = cursor.fetchone()
            
            cursor.execute("SELECT COUNT(DISTINCT set_name) as unique_sets FROM cards")
            sets = cursor.fetchone()
            
            cursor.execute("SELECT COUNT(*) as foils FROM cards WHERE foil_treatment = 'Foil'")
    ...
    def cleanup_old_data(self, days_to_keep: int = 30) -> None:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("DELETE FROM dashboard_cache WHERE expires_at < datetime('now')")
