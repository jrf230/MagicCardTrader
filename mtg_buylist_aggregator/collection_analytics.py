...
class CollectionAnalytics:
    ...
    def _analyze_performance(self, card_prices_list: List[CardPrices]) -> Dict:
        ...
        for card_prices in card_prices_list:
            ...
            history = self.price_history.get_card_history(
                card.name, card.set_name, days=self.config["growth_period_days"]
            )

            if history:
                historical_prices = [
                    entry["best_bid"] for entry in history if entry.get("best_bid")
                ]
                if historical_prices:
                    ...
                    growth_rate = (
                        ((current_value - historical_value) / historical_value) * 100
        ...
        overall_growth_rate = (
            ((total_current_value - total_historical_value) / total_historical_value)
            * 100
            if total_historical_value > 0
            else 0
        )
    ...
    def _assess_risk(self, card_prices_list: List[CardPrices]) -> Dict:
        ...
        for risk_level, data in risk_categories.items():
            if total_value > 0:
                risk_metrics[risk_level] = {
                    ...
                    "value": data["value"],
                    "count": data["count"],
                    "weighted_risk": data["value"]
                    * self.config["risk_categories"][risk_level],
                }
            else:
                risk_metrics[risk_level] = {
                    "percentage": 0.0,
        ...
        total_weighted_risk = sum(
            metrics["weighted_risk"] for metrics in risk_metrics.values()
        )
    ...
    def _identify_top_performers(
        self, card_prices_list: List[CardPrices], limit: int = 10
    ) -> List[Dict]:
        ...
        for card_prices in card_prices_list:
            ...
            if history:
                ...
                if historical_prices:
                    ...
                    if (
                        growth_rate
                        > self.config["performance_benchmarks"]["top_performer"] * 100
                    ):
                        performers.append(
                            {
                                "card": {
        ...
        for card_prices in card_prices_list:
            ...
            if history:
                ...
                if historical_prices:
                    ...
                    if (
                        growth_rate
                        < self.config["performance_benchmarks"]["underperformer"] * 100
                    ):
    ...
    def _analyze_set_performance(self, card_prices_list: List[CardPrices]) -> Dict:
        ...
        for set_name, data in set_performance.items():
            if data["historical_value"] > 0:
                data["growth_rate"] = (
                    (data["current_value"] - data["historical_value"])
    ...
    def _analyze_condition_impact(self, card_prices_list: List[CardPrices]) -> Dict:
        ...
        """
        condition_data = defaultdict(
            lambda: {"cards": [], "total_value": 0.0, "card_count": 0, "avg_price": 0.0}
        )
    ...
    def _assess_card_risk(self, card: Card, card_prices: CardPrices) -> str:
        ...
        if card.set_name and any(
            keyword in card.set_name.lower()
    ...
    def _calculate_period_growth(
        self, card_prices_list: List[CardPrices], days: int
    ) -> Dict:
        ...
        for card_prices in card_prices_list:
            ...
            # Get historical value
            history = self.price_history.get_card_history(
                card.name, card.set_name, days=days
            )
    ...
    def _generate_diversification_recommendations(
        self, set_concentration: Dict, hhi: float
    ) -> List[str]:
        ...
        if hhi > 0.4:
            recommendations.append(
                "Consider diversifying across more sets to reduce concentration risk"
            )

        # Find most concentrated sets
        top_sets = sorted(set_concentration.items(), key=lambda x: x[1], reverse=True)[
            :3
        ]
        for set_name, concentration in top_sets:
            if concentration > 0.2:
                recommendations.append(
                    f"Reduce exposure to {set_name} (currently {concentration:.1%} of collection)"
                )
    ...
    def _generate_risk_recommendations(
        self, risk_metrics: Dict, overall_risk_score: float
    ) -> List[str]:
        ...
        if high_risk_pct > 30:
            recommendations.append(
                f"High-risk cards represent {high_risk_pct:.1f}% of collection - consider rebalancing"
            )
    ...
    def generate_analytics_report(self, analysis: Dict) -> str:
        ...
        report_lines.append("-" * 40)
        report_lines.append(
            f"Overall Growth Rate: {performance['overall_growth_rate']:+.1f}%"
        )
        report_lines.append(
            f"Performance Level: {performance['performance_level'].title()}"
        )
        report_lines.append(
            f"Growth Amount: ${performance['overall_growth_amount']:+.2f}"
        )
        report_lines.append("")
        ...
        report_lines.append("-" * 40)
        report_lines.append(
            f"Diversification Level: {diversification['diversification_level'].title()}"
        )
        report_lines.append(f"HHI Index: {diversification['hhi_index']:.3f}")
        report_lines.append(
            f"Number of Sets: {len(diversification['set_distribution'])}"
        )
        report_lines.append("")
        ...
        for risk_level, data in risk["risk_categories"].items():
            if data["percentage"] > 0:
                report_lines.append(
                    f"{risk_level.replace('_', ' ').title()}: {data['percentage']:.1f}% (${data['value']:.2f})"
                )
        report_lines.append("")
        ...
        if top_performers:
            ...
            for i, performer in enumerate(top_performers[:5], 1):
                card = performer["card"]
                report_lines.append(
                    f"{i}. {card['name']} ({card['set_name']}): {performer['growth_rate']:+.1f}%"
                )
        report_lines.append("")
