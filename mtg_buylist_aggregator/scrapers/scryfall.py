    ...
    def search_card(self, card: Card) -> List[PriceData]:
        ...
        try:
            ...
            if card.set_name:
                ...
                try:
                    set_params = {"exact": card.name, "set": card.set_name.lower().replace(" ", "-")}
    ...
    def _parse_scryfall_data(self, data: Dict, card: Card) -> List[PriceData]:
        ...
        try:
            ...
            if data.get("object") == "list":
                self.logger.debug(f"Scryfall returned a list for {card.name}. Searching for best match...")
                ...
                for card_data in data.get("data", []):
                    if card_data.get("set_name", "").lower() == card.set_name.lower():
                        data = card_data
                        self.logger.debug(f"Found exact set match in list: {card.set_name}")
                        break
                else:
                    # If no exact match, use the first result as a fallback
                    data = data.get("data", [{}])[0]
                    self.logger.debug("No exact set match found, using first result from list.")

            # Get prices
            prices = data.get("prices", {})
            if not prices:
                self.logger.warning(f"No price data in Scryfall response for {card.name}")
                return []
            ...
            self.logger.error(f"Error parsing Scryfall data for {card.name}: {e}. Data: {data}")
            return []
