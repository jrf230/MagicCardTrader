...
class ScraperManager:
            ...
            if is_pkg:
                try:
                    module = importlib.import_module(
                        f"mtg_buylist_aggregator.scrapers.plugins.{name}"
                    )
    ...
    def __init__(
        self, max_workers: int = 3, use_mock: bool = False, plugins_dir: str = None
    ):
    ...
    def _initialize_scrapers(self) -> None:
        ...
        for vendor, scraper_cls in vendor_classes.items():
            try:
                ...
                logger.info(
                    f"Initialized {vendor} scraper ({'mock' if self.use_mock else 'real'})"
                )
            except Exception as e:
    ...
    def _get_vendor_classes(self) -> Dict[str, type]:
        ...
        if self.use_mock:
            if not self._MOCK_SCRAPER_REGISTRY:
                ...
                self.register_mock_scraper(
                    "Star City Games", lambda: MockScraper("Star City Games")
                )
                self.register_mock_scraper(
                    "Card Kingdom", lambda: MockScraper("Card Kingdom")
                )
                self.register_mock_scraper(
                    "BeatTheBuylist", lambda: MockScraper("BeatTheBuylist")
                )
                self.register_mock_scraper(
                    "eBay Recent Sales", lambda: MockScraper("eBay Recent Sales")
                )
                self.register_mock_scraper(
                    "TCG Player", lambda: MockScraper("TCG Player")
                )
                self.register_mock_scraper(
                    "Channel Fireball", lambda: MockScraper("Channel Fireball")
                )
                self.register_mock_scraper(
                    "CoolStuffInc", lambda: MockScraper("CoolStuffInc")
                )
                ...
                try:
                    ...
                    if price_data_list:
                        ...
                        for price_data in price_data_list:
                            logger.debug(
                                f"Got price from {scraper_name}: ${price_data.price} ({price_data.price_type})"
                            )
                    else:
    ...
    def get_collection_prices(self, cards: List[Card]) -> List[CardPrices]:
        ...
        """
        logger.info(
            f"Getting prices for {len(cards)} cards from {len(self.scrapers)} vendors"
        )
        ...
        for i, card in enumerate(cards, 1):
            logger.info(
                f"Processing card {i}/{total_cards}: {card.name} ({card.set_name})"
            )
            ...
            try:
                    ...
                    for future in as_completed(future_to_scraper):
                        ...
                        try:
                            ...
                            if price_data_list:
                                ...
                                for price_data in price_data_list:
                                    logger.debug(
                                        f"Got price from {vendor} for {card.name}: ${price_data.price} ({price_data.price_type})"
                                    )
                            else:
                                logger.warning(
                                    f"No price found for {card.name} from {vendor}"
                                )
                                error_log.append((card.name, vendor, "No price found"))
                        except Exception as e:
                            logger.error(
                                f"Error getting price from {vendor} for {card.name}: {e}"
                            )
                ...
                if card_prices.best_bid:
                    ...
                    logger.info(
                        f"  Best buylist price: ${best_price} from {best_vendor}"
                    )
                else:
    ...
    def test_scrapers(self) -> Dict[str, bool]:
        ...
        """
        test_card = Card(name="Sol Ring", set_name="Commander 2021", quantity=1)
        ...
        for vendor_name, scraper in self.scrapers.items():
            try:
                ...
                logger.info(
                    f"  {status} {vendor_name}: {'Working' if price_data else 'No price found'}"
                )
            except Exception as e:
    ...
    def list_vendors(self) -> List[str]:
        ...
        return list(self._SCRAPER_REGISTRY.keys()) + list(
            self._MOCK_SCRAPER_REGISTRY.keys()
        )
