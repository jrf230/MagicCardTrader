...
class HarlequinGamesScraper(BaseScraper):
    ...
    def __init__(self):
        super().__init__("Harlequin Games", "https://www.harlequins-games.com/advanced_search")
    ...
    def search_card(self, card: Card) -> Optional[PriceData]:
        ...
        try:
            ...
            if manual_data:
                logger.debug(f"Harlequin Games: Using manual data for {card.name} ({card.set_name})")
                return manual_data
            
            # If no manual data, try web scraping
            logger.debug(f"Harlequin Games: Attempting to scrape {card.name} ({card.set_name})")
            return self._attempt_scraping(card)
                
        except Exception as e:
            logger.debug(f"Harlequin Games: Error searching for {card.name} ({card.set_name}): {e}")
            return None
    ...
    def _attempt_scraping(self, card: Card) -> Optional[PriceData]:
        ...
        try:
            ...
            logger.debug(f"Harlequin Games: Web scraping not implemented for {card.name} ({card.set_name})")
            return None
            
        except Exception as e:
            logger.debug(f"Harlequin Games: Scraping failed for {card.name} ({card.set_name}): {e}")
            return None
    ...
    def _get_sell_price(self, card: Card) -> Optional[PriceData]:
        ...
        try:
            ...
            logger.debug(f"Harlequin Games: Error getting sell price for {card.name}: {e}")
            return None
