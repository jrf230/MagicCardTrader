"""Troll Trader Cards scraper for buylist and sell list prices."""
...
class TrollTraderScraper(BaseScraper):
    """Scraper for Troll Trader Cards buylist and sell list prices."""
    
    def __init__(self):
        super().__init__("Troll Trader Cards", "https://www.trolltradercards.com/buylist")
        ...
        self.manual_data = {
            "Rhystic Study": {
                "Prophecy": {
                    "buylist_cash_gbp": 23.23,
                    "buylist_credit_gbp": 30.20,
            ...
            "Sol Ring": {
                "Commander 2021": {
                    "buylist_cash_gbp": 8.50,
                    "buylist_credit_gbp": 11.05,
                    "sell_price_gbp": 15.00,
                    "condition": "Near Mint",
                    "quantity_limit": 100
                }
            },
    ...
    def search_card(self, card: Card) -> Optional[PriceData]:
        """Search for a card and return buylist price data from Troll Trader Cards."""
        try:
            # Check manual data
            manual_data = self._get_manual_data(card)
            if manual_data:
                logger.debug(f"Troll Trader: Using manual data for {card.name} ({card.set_name})")
                return manual_data
            
            # If no manual data, try web scraping
            logger.debug(f"Troll Trader: Attempting to scrape {card.name} ({card.set_name})")
            return self._attempt_scraping(card)
                
        except Exception as e:
            logger.debug(f"Troll Trader: Error searching for {card.name} ({card.set_name}): {e}")
            return None
    ...
    def _get_manual_data(self, card: Card) -> Optional[PriceData]:
        ...
        if card_data:
            ...
            cash_price_usd = self._convert_gbp_to_usd(card_data.get("buylist_cash_gbp", 0))
            credit_price_usd = self._convert_gbp_to_usd(card_data.get("buylist_credit_gbp", 0))
            ...
            if all_conditions:
                return PriceData(
                    ...
                    all_conditions={
                        ...
                        "note": "Troll Trader Cards buylist prices (GBP converted to USD)",
    ...
    def _attempt_scraping(self, card: Card) -> Optional[PriceData]:
        """Attempt to scrape Troll Trader Cards buylist."""
        try:
            # For now, return None as web scraping is not implemented
            # The site appears to be a JavaScript-heavy SPA
            logger.debug(f"Troll Trader: Web scraping not implemented for {card.name} ({card.set_name})")
            return None
            
        except Exception as e:
            logger.debug(f"Troll Trader: Scraping failed for {card.name} ({card.set_name}): {e}")
            return None
        ...
        try:
            ...
            if card_data and card_data.get("sell_price_gbp"):
                ...
                return PriceData(
                    ...
                    last_price_update=datetime.now(),
                    all_conditions={
                        "search_url": "https://www.trolltradercards.com",
            ...
            logger.debug(f"Troll Trader: Error getting sell price for {card.name}: {e}")
            return None

    def get_buylist(self) -> List[PriceData]:
        """Get the complete buylist from Troll Trader Cards."""
        logger.warning("Troll Trader Cards: Full buylist scraping not implemented.")
        return []

    def __str__(self) -> str:
        return f"Troll Trader Cards Scraper ({self.base_url})" 