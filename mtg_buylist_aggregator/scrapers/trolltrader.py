...
class TrollTraderScraper(BaseScraper):
    ...
    def __init__(self):
        super().__init__(
            "Troll Trader Cards", "https://www.trolltradercards.com/buylist"
        )
    ...
    def search_card(self, card: Card) -> Optional[PriceData]:
        ...
        try:
            ...
            if manual_data:
                logger.debug(
                    f"Troll Trader: Using manual data for {card.name} ({card.set_name})"
                )
                return manual_data

            # If no manual data, try web scraping
            logger.debug(
                f"Troll Trader: Attempting to scrape {card.name} ({card.set_name})"
            )
            return self._attempt_scraping(card)

        except Exception as e:
            logger.debug(
                f"Troll Trader: Error searching for {card.name} ({card.set_name}): {e}"
            )
            return None
    ...
    def _attempt_scraping(self, card: Card) -> Optional[PriceData]:
        ...
        try:
            ...
            logger.debug(
                f"Troll Trader: Web scraping not implemented for {card.name} ({card.set_name})"
            )
            return None

        except Exception as e:
            logger.debug(
                f"Troll Trader: Scraping failed for {card.name} ({card.set_name}): {e}"
            )
            return None
