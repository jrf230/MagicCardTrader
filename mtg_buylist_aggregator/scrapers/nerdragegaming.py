...
class NerdRageGamingScraper(BaseScraper):
    ...
    def __init__(self):
        super().__init__("Nerd Rage Gaming", "https://www.nerdragegaming.com/nrgbuylist")
    ...
    def search_card(self, card: Card) -> Optional[PriceData]:
        ...
        try:
            ...
            if manual_data:
                logger.debug(f"Nerd Rage Gaming: Using manual data for {card.name} ({card.set_name})")
                return manual_data
            
            # If no manual data, try web scraping
            logger.debug(f"Nerd Rage Gaming: Attempting to scrape {card.name} ({card.set_name})")
            return self._attempt_scraping(card)
                
        except Exception as e:
            logger.debug(f"Nerd Rage Gaming: Error searching for {card.name} ({card.set_name}): {e}")
            return None
    ...
    def _attempt_scraping(self, card: Card) -> Optional[PriceData]:
        ...
        try:
            ...
            logger.debug(f"Nerd Rage Gaming: Web scraping not implemented for {card.name} ({card.set_name})")
            return None
            
        except Exception as e:
            logger.debug(f"Nerd Rage Gaming: Scraping failed for {card.name} ({card.set_name}): {e}")
            return None
    ...
    def _get_sell_price(self, card: Card) -> Optional[PriceData]:
        ...
        try:
            ...
            logger.debug(f"Nerd Rage Gaming: Error getting sell price for {card.name}: {e}")
            return None
