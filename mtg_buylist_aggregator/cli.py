...
def list_cards_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.verbose:
            ...
            print("-" * 120)
            print(
                f"{'Card Name':<25} {'Set':<20} {'#':<3} {'Rarity':<8} {'Foil':<6} {'Condition':<8} {'Variant':<12}"
            )
            print("-" * 120)
            ...
            for card in cards:
                ...
                print(
                    f"{card.name:<25} {card.set_name:<20} {card.card_number or '':<3} {rarity_str:<8} {foil_str:<6} {condition_str:<8} {variant_str:<12}"
                )
        else:
            print(f"\nCollection ({len(cards)} unique cards):")
            print("-" * 80)
            ...
            for card in cards:
                foil_str = "Yes" if card.is_foil() else "No"
                print(
                    f"{card.name:<30} {card.set_name:<25} {card.quantity:<5} {foil_str:<5}"
                )
...
def find_best_prices_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.dry_run:
            ...
            print(
                f"Would export to: {args.export_csv if args.export_csv else 'No export'}"
            )
            return 0
        ...
        working_scrapers = [
            vendor for vendor, working in test_results.items() if working
        ]
        ...
        print(
            f"✓ Found {len(working_scrapers)} working scrapers: {', '.join(working_scrapers)}"
        )
        ...
        if output_format == "json":
            ...
            if config.get("track_history", True) and config.get("show_trends", True):
                try:
                    history = PriceHistory()
                    trend_report = history.generate_trend_report(
                        card_prices_list, days=30
                    )
                    if trend_report:
                        print("\n" + trend_report)
                except Exception as e:
        ...
        if args.export_csv:
            filename = (
                args.export_csv if args.export_csv != "auto" else "price_report.csv"
            )
...
def history_show_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.card_name and args.set_name:
            ...
            print(
                f"Price History for {args.card_name} ({args.set_name}) - Last {args.days} days"
            )
            print("=" * 80)
            for entry in entries:
                ...
                for vendor, price_data in entry["prices"].items():
                    print(
                        f"  {vendor}: ${price_data['price']:.2f} ({price_data['condition']})"
                    )
                if "best_price" in entry:
                    print(
                        f"  Best: ${entry['best_price']:.2f} from {entry['best_vendor']}"
                    )
        else:
            # Show statistics
...
def history_trends_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.card_name and args.set_name:
            ...
            analysis = history.analyze_price_trends(
                args.card_name, args.set_name, args.days
            )
            ...
            print(
                f"Price Trends for {args.card_name} ({args.set_name}) - Last {args.days} days"
            )
            print("=" * 80)
            ...
            if "best_price" in analysis["trends"]:
                ...
                trend_symbol = (
                    "↗️"
                    if trend["trend"] == "up"
                    else "↘️" if trend["trend"] == "down" else "→"
                )
                print(
                    f"Best Price: {trend_symbol} ${trend['change']:+.2f} ({trend['change_percent']:+.1f}%)"
                )
                print(
                    f"  From: ${trend['first_price']:.2f} → To: ${trend['last_price']:.2f}"
                )
                print()
            ...
            for vendor, change in analysis["price_changes"].items():
                trend_symbol = (
                    "↗️"
                    if change["trend"] == "up"
                    else "↘️" if change["trend"] == "down" else "→"
                )
                print(
                    f"{vendor}: {trend_symbol} ${change['change']:+.2f} ({change['change_percent']:+.1f}%)"
                )
                print(
                    f"  From: ${change['first_price']:.2f} → To: ${change['last_price']:.2f}"
                )
        else:
...
def history_cleanup_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(
            f"Before cleanup: {stats_before['total_entries']} entries for {stats_before['total_cards']} cards"
        )
        ...
        print(
            f"After cleanup: {stats_after['total_entries']} entries for {stats_after['total_cards']} cards"
        )
...
def hot_cards_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if getattr(args, "dry_run", False):
            ...
            print(
                f"Would export to: {args.export_csv if getattr(args, 'export_csv', None) else 'No export'}"
            )
            return 0
        scraper_manager = ScraperManager(use_mock=use_mock)
                    ...
                    writer.writerow(
                        {
                            ...
                            "Trend": card_analysis["trend_direction"]
                            .replace("_", " ")
                            .title(),
                            "Recommendation": card_analysis["recommendation"],
                            "Risk Level": (
                                card_analysis["risk_factors"][0]
                                if card_analysis["risk_factors"]
                                else "None"
                            ),
...
def recommendations_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        recommendations = recommendation_engine.generate_recommendations(
            card_prices_list, collection_summary
        )
...
def analytics_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        analysis = collection_analytics.analyze_collection(
            card_prices_list, collection_summary
        )
...
def market_analysis_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        scraper_manager = ScraperManager(
            max_workers=args.max_workers, use_mock=args.use_mock
        )
        ...
        print(
            f"Analysis Date: {analysis['analysis_date'].strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        if args.verbose:
            print(f"\n=== DETAILED INSIGHTS ===")
            insights = sorted(
                analysis["insights"], key=lambda x: abs(x.price_spread), reverse=True
            )

            print(
                f"{'Card':<25} {'Market':<8} {'Buylist':<8} {'Spread':<8} {'Trend':<8} {'Risk':<6} {'Rec':<12}"
            )
            print("-" * 85)

            for insight in insights[:20]:  # Show top 20
                trend_str = (
                    f"{insight.trend_percentage:+.1f}%"
                    if insight.trend_percentage != 0
                    else "0%"
                )
                print(
                    f"{insight.card.name:<25} "
...
def research_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.vendor_report:
            ...
            print(
                f"Analysis Date: {insights['analysis_date'].strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            for source in sources:
                print(
                    f"• {source['name']} ({source['type']}) - {source['description']}"
                )
...
def create_parent_parser() -> argparse.ArgumentParser:
    ...
    parent.add_argument(
        "--config", type=str, default=None, help="Path to alternate config file"
    )
    parent.add_argument(
        "--dry-run",
        action="store_true",
...
def create_parser() -> argparse.ArgumentParser:
    ...
    add_parser.add_argument(
        "--quantity", "-q", type=int, default=1, help="Quantity (default: 1)"
    )
    add_parser.add_argument("--foil", "-f", action="store_true", help="Foil version")
    add_parser.add_argument(
        "--collection", "-c", default="collection.csv", help="Collection file path"
    )

    # Remove card command
    remove_parser = subparsers.add_parser(
        "remove-card", help="Remove a card from your collection"
    )
    ...
    remove_parser.add_argument(
        "--quantity", "-q", type=int, help="Quantity to remove (default: all)"
    )
    remove_parser.add_argument(
        "--collection", "-c", default="collection.csv", help="Collection file path"
    )
    ...
    list_parser = subparsers.add_parser(
        "list-cards", help="List all cards in your collection"
    )
    ...
    search_parser = subparsers.add_parser(
        "search-cards", help="Search for cards in your collection"
    )
    ...
    find_best_prices_parser = subparsers.add_parser(
        "find-best-prices",
        parents=[parent],
        help="Find best buylist prices for collection",
    )
    find_best_prices_parser.add_argument(
        "--collection", "-c", default="collection.csv", help="Collection file path"
    )
    find_best_prices_parser.add_argument(
        "--use-mock", action="store_true", help="Use mock scrapers for testing"
    )
    find_best_prices_parser.add_argument(
        "--output-format",
    ...
    find_best_prices_parser.add_argument(
        "--export-csv", nargs="?", const="auto", help="Export results to CSV file"
    )
    find_best_prices_parser.set_defaults(func=find_best_prices_command)

    # Hot cards command
    hot_cards_parser = subparsers.add_parser(
        "hot-cards", help="Detect hot cards with price spikes"
    )
    hot_cards_parser.add_argument(
        "--collection", "-c", default="collection.csv", help="Collection file path"
    )
    hot_cards_parser.add_argument(
        "--use-mock", action="store_true", help="Use mock scrapers for testing"
    ...
    hot_cards_parser.add_argument(
        "--days", "-d", type=int, default=7, help="Days to analyze for price spikes"
    )
    hot_cards_parser.add_argument(
        "--export-csv", nargs="?", const="auto", help="Export results to CSV file"
    )
    ...
    recommendations_parser = subparsers.add_parser(
        "recommendations", help="Get buy/sell/hold recommendations"
    )
    ...
    analytics_parser = subparsers.add_parser(
        "analytics", help="Analyze your collection in depth"
    )
    ...
    history_show_parser.add_argument(
        "--days", "-d", type=int, default=30, help="Number of days to show"
    )

    # History trends
    history_trends_parser = history_subparsers.add_parser(
        "trends", help="Show price trends"
    )
    history_trends_parser.add_argument("--card-name", "-n", help="Card name")
    history_trends_parser.add_argument("--set-name", "-s", help="Set name")
    history_trends_parser.add_argument(
        "--days", "-d", type=int, default=30, help="Number of days to show"
    )

    # History cleanup
    history_cleanup_parser = history_subparsers.add_parser(
        "cleanup", help="Clean up price history"
    )
    history_cleanup_parser.add_argument(
        "--days", "-d", type=int, default=30, help="Number of days to clean up"
    )

    # Vendor health command
    vendor_health_parser = subparsers.add_parser(
        "vendor-health", help="Check health of all vendor scrapers"
    )
    vendor_health_parser.add_argument(
        "--use-mock", action="store_true", help="Use mock scrapers for testing"
    )
    vendor_health_parser.set_defaults(func=vendor_health_command)

    # List vendors command
    list_vendors_parser = subparsers.add_parser(
        "list-vendors", help="List all available vendor scrapers"
    )
    list_vendors_parser.add_argument(
        "--use-mock", action="store_true", help="Use mock scrapers for testing"
    )
    ...
    market_analysis_parser = subparsers.add_parser(
        "market-analysis", help="Perform market analysis on your collection"
    )
    market_analysis_parser.add_argument(
        "--collection", "-c", default="collection.csv", help="Collection file path"
    )
    market_analysis_parser.add_argument(
        "--use-mock", action="store_true", help="Use mock scrapers for testing"
    ...
    research_parser = subparsers.add_parser(
        "research", help="Research MTG vendors and market data"
    )
    research_parser.add_argument(
        "--vendor-report",
        action="store_true",
        help="Generate detailed vendor analysis report",
    )
    research_parser.add_argument(
        "--pricing-sources", action="store_true", help="Show pricing data sources"
    )
    research_parser.add_argument(
        "--market-insights", action="store_true", help="Show market insights and trends"
    )
    research_parser.add_argument(
        "--export-csv", nargs="?", const="auto", help="Export vendor data to CSV"
    )
    ...
    update_prices_parser = subparsers.add_parser(
        "update-prices",
        help="Update price cache for all cards in collection (daily update)",
    )
...
def main() -> int:
    ...
    if args.command == "add-card":
        ...
        if args.config_command == "show":
            ...
            print(
                "Unknown config command. Use 'config show', 'config set', or 'config reset'"
            )
            return 1
        ...
        if args.history_command == "show":
            ...
            print(
                "Unknown history command. Use 'history show', 'history trends', or 'history cleanup'"
            )
            return 1
