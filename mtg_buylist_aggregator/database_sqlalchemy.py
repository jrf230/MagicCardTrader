    ...
    def migrate_from_sqlite(self, old_db_path: str):
        ...
        try:
            ...
            with self.get_session() as session:
                # Migrate cards
                old_cursor.execute(
                    "SELECT name, set_name, foil, condition, quantity FROM cards"
                )
                ...
                for row in old_cursor.fetchall():
                    vendor = Vendor(name=row[0], url=row[1], is_active=bool(row[2]))
                    session.add(vendor)

                # Migrate price cache
                old_cursor.execute(
                    "SELECT card_name, vendor_name, price, condition, foil, timestamp FROM price_cache"
                )
                for row in old_cursor.fetchall():
                    # Find or create card
