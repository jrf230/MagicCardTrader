...
class PriceHistory:
    ...
    def get_card_history(
        self, card_name: str, set_name: str, days: int = 30
    ) -> List[Dict]:
    ...
    def analyze_price_trends(
        ...
        best_prices = [
            entry["best_price"] for entry in history if "best_price" in entry
        ]
        if len(best_prices) >= 2:
            ...
            best_change_percent = (
                (best_change / first_best) * 100 if first_best > 0 else 0
            )
            ...
            analysis["trends"]["best_price"] = {
                ...
                "trend": (
                    "up" if best_change > 0 else "down" if best_change < 0 else "stable"
                ),
            }
    ...
    def generate_trend_report(
        self, card_prices_list: List[CardPrices], days: int = 30
    ) -> str:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "average_entries_per_card": (
                total_entries / total_cards if total_cards > 0 else 0
            ),
        }
