    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, set_info in enumerate(data.get("sets", [])[:10]):
                print(
                    f"  {i+1}. {set_info['name']} ({set_info['code']}) - {set_info['released_at'][:4]}"
                )
...
def test_search_autocomplete():
    ...
    try:
        response = requests.get(
            f"http://localhost:8080/api/search-cards-autocomplete?q={query}"
        )
        print(f"Status Code: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            for i, card in enumerate(data.get("cards", [])[:3]):
                print(
                    f"  {i+1}. {card['name']} - {card['set_name']} ({card['set_code']})"
                )
